...
class Embeddings(nn.Module):
    ...
    def __init__(self, config,channels = 3):
        ...
        #                            nn.Conv2d(in_channels=3, out_channels=config.hidden_size,kernel_size=patch_size,stride=patch_size,bias=False),
        #                            torch.nn.BatchNorm2d(config.hidden_size),
        #                            torch.nn.ReLU(),
        #                            nn.Conv2d(in_channels=config.hidden_size, out_channels=config.hidden_size,kernel_size=[1,1],stride=[1,1],bias=False)
        #                        )
    ...
    def forward(self, input_leads,attention_show_flg=False):
        ...
        mim: Masked Image Modeling mask
...
class Layer(nn.Module):
    ...
    def forward(self, hidden_states, attention_show_flg=False):
        ...
        mim：Transformerのマスクと同じ働きのマスキング
...
class Attention(nn.Module):
    ...
    def forward(self, input_tensor, attention_show_flg=False):
        ...
        mim：Transformerのマスクと同じ働きのマスキングです
        attention_show_flg：Self-Attentionの重みを返すかのフラグ
        '''
        if attention_show_flg == True:
            '''attention_showのときは、attention_probsもリターンする'''
...
class SelfAttention(nn.Module):
    ...
    def __init__(self, config, method='dw_bn',kernel_size=[1,3], stride_kv=1,stride_q=1,padding_kv=[0,1], padding_q=[0,1]):
        ...
        self.conv_proj_q = self._build_projection(kernel_size, padding_q, stride_q, method)
        self.conv_proj_k = self._build_projection(kernel_size, padding_kv, stride_kv, method)
        self.conv_proj_v = self._build_projection(kernel_size, padding_kv,stride_kv, method)
        # Self-Attentionの特徴量を作成する全結合層
        self.proj_q = nn.Linear(self.hidden_size, self.hidden_size)
        self.proj_k = nn.Linear(self.hidden_size, self.hidden_size)
        self.proj_v = nn.Linear(self.hidden_size, self.hidden_size)
    ...
    def _build_projection(self,kernel_size,padding,stride,method):
        if method == 'dw_bn':
            proj = nn.Sequential(OrderedDict([
                ('conv', nn.Conv2d(self.hidden_size,self.hidden_size,kernel_size=kernel_size,padding=padding,stride=stride)),
                ('bn', nn.BatchNorm2d(self.hidden_size)),
    ...
    def forward_conv(self, x):
        cls_token, x = torch.split(x, [self.num_lead+1, self.patch_num_h*self.num_lead], 1)

        x = rearrange(x, 'b (h w) c -> b c h w', h=self.num_lead, w=self.patch_num_h) # row: num view column: view patch
    ...
    def forward(self, hidden_states, attention_show_flg=False):
        q, k, v = self.forward_conv(hidden_states)

        q = rearrange(self.proj_q(q), 'b t (h d) -> b h t d', h=self.num_heads)
        k = rearrange(self.proj_k(k), 'b t (h d) -> b h t d', h=self.num_heads)
        v = rearrange(self.proj_v(v), 'b t (h d) -> b h t d', h=self.num_heads)

        attention_scores = torch.einsum('bhlk,bhtk->bhlt', [q, k]) / math.sqrt(self.attention_head_size)
        ...
        # attentionの制限
        if self.howatt == 0:
            attention_scores[:,:,0:self.num_lead+1,:] += cls_lead_mask.to(hidden_states.device) 
        else:
        ...
        context_layer = torch.einsum('bhlt,bhtv->bhlv', [attention_probs, v])
...
class MCiT(nn.Module):
    ...
    def forward(self, input_leads, output_all_encoded_layers=False, attention_show_flg=False,output_all_encoded_layers_show=False):
        '''
        ...
        if output_all_encoded_layers_show:
            return torch.stack(encoded_layers)
        
        if self.cls_feature == 0: # 入力の[CLS]の特徴量を使用して分類
            vec_cls = encoded_layers[:, 0, :].view(-1, self.features_)
            out = self.clssifier_cls(vec_cls)
        elif self.cls_feature == 1: # [CLS] + 全[SEG]の特徴量を使用して分類
            vec_cls = encoded_layers[:, 0:self.num_lead+1, :].view(-1, self.features_)
            out = self.clssifier_cls(vec_cls)
        elif self.cls_feature == 2:
            vec_cls = encoded_layers[:, 0:self.num_lead+1, :].view(-1, self.features_)
            vec_cls = self.mlphead(vec_cls)
            out = self.clssifier_cls(vec_cls)
        elif self.cls_feature == 3:
            pool_view = encoded_layers[:, 1:self.num_lead+1, :]
            pool_cls = torch.mean(pool_view,1)
            vec_cls = torch.cat((encoded_layers[:, 0, :],pool_cls),dim=1)
            out = self.clssifier_cls(vec_cls)
