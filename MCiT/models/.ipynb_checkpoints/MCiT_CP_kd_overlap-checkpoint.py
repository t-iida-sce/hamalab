...
class Embeddings(nn.Module):
    ...
    def __init__(self, config,channels = 3):
        ...
        stride_size = (int(self.imagesize[0]/self.patch_num_v), int(self.imagesize[1]/self.patch_num_h/self.overlap)) # 画像における列方向に分割, 分割された数がembeddingする数となる
        self.patch_embedding = nn.Conv2d(in_channels=3, out_channels=config.hidden_size,kernel_size=patch_size,stride=stride_size,bias=False)
        
        if self.pos_embd == 0:  # 全てのチャートに対して, 昇順のパラメータを作成, 位置埋め込み
            self.max_position_num = self.num_lead * int((self.imagesize[1] - (patch_size[1]-1) -1 )/stride_size[1]+1) + self.num_lead+1                                            
            self.position_embeddings = nn.Embedding(self.max_position_num, config.hidden_size)
        else:         # 単一チャートに対して, 昇順のパラメータを作成, 位置埋め込み
            self.patch_position_num = int((self.imagesize[1] - (patch_size[1]-1) -1 )/stride_size[1]+1)
...
class SelfAttention(nn.Module):
    ...
    def __init__(self, config, method='dw_bn',kernel_size=[1,3], stride_kv=1,stride_q=1,padding_kv=[0,1], padding_q=[0,1]):
        ...
        self.overlap = 2
        self.patches_num = self.patch_num_v*self.patch_num_h*self.overlap-1
    ...
    def forward_conv(self, x):
        cls_token, x = torch.split(x, [self.num_lead+1, self.patches_num*self.num_lead], 1)

        x = rearrange(x, 'b (h w) c -> b c h w', h=self.num_lead, w=self.patches_num) # row: num view column: view patch
...
def gelu(x):
    '''Gaussian Error Linear Unitという活性化関数
    '''
...
class te(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.overlap = 2
        self.patches_num = self.patch_num_v*self.patch_num_h*self.overlap-1
        self.max_position_embeddings = self.num_lead*self.patches_num+(self.num_lead+1) # 誘導画像数*パッチ数 + cls + seg
...
class MCiT(nn.Module):
    ...
    def forward(self, input_leads, output_all_encoded_layers=True, attention_show_flg=False,output_all_encoded_layers_show=False):
        ...
        if self.cls_feature == 0:
            vec_cls = encoded_layers[-1,:,0, :].clone()
