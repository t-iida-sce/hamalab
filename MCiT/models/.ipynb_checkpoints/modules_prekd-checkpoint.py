...
def train_model(model, train_loader, test_loader,num_epochs,optimizer,criterion,save_dir):
    ...
    for epoch in range(num_epochs):
        ...
        train_l1_loss = 0 # L1 loss
        test_loss = 0
        test_l1_loss = 0 # L1 loss
        train_acc = 0
        test_acc = 0
...
def train_kd(s_model, t_model, train_loader, test_loader,num_epochs,optimizer,loss_fn,save_dir):
    ...
    for epoch in range(num_epochs):
        ...
        for i,(ecg,l,p,num) in enumerate(train_loader):
            ...
            for j in range(ecg.size(1)):
                with torch.no_grad():
                    ...
                    #print(embed[:,j+1,:])
                loss += loss_fn.kd(embed[:,j+1,:], out_t_model)
        ...
        if test_loss < test_loss_min:
            ...
            torch.save(early_model.state_dict(), os.path.join(save_dir, 'vit_kd_earlystoped.pth'))
            print(f'\nEarly Stopping Total epochs: {epoch}. Best epoch: {best_epoch} with loss: {test_loss_min:.2f} ')
                
        print('Epoch {}/{} | train/test Loss: {:.4f}/{:.4f} '.format(epoch+1, num_epochs,train_loss,test_loss))

        history.append([train_loss,test_loss])
 
    history = np.array(history)
    save_history(history,save_dir)
    torch.save(s_model.state_dict(), os.path.join(save_dir, 'vit_kd.pth'))
