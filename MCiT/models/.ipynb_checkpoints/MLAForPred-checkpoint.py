...
class TEForPred(nn.Module):
    ...
    def __init__(self, net,config,task=5):
        ...
        if config.classifier_conf.cls == 0:
            ...
            self.features_ = config.model_conf.hidden_size*self.num_lead
            self.clssifier_cls = nn.Linear(in_features=self.features_, out_features=task)
    ...
    def forward(self, input_leads, attention_mask=None, output_all_encoded_layers=False, attention_show_flg=False):
        ...
        if self.cls_feature == 0:
            ...
            vec_cls = encoded_layers[:, 1:self.num_lead+1, :]
        elif self.cls_feature == 2:
            vec_cls = encoded_layers[:, 0:self.num_lead+1, :]
...
def accuracy(device,model_,loader,eval,save_dir,label_name,config,attention_show_flg=False):
    ...
    with torch.no_grad():
        ...
        for ecg, label,p,num in loader:
            ...
            outputs, attention_map = model_(ecg, attention_mask=None,attention_show_flg=attention_show_flg)
            if attention_show_flg == True:
        ...
        # 2値分類用
        if config.classifier_conf.cls_task == 5:
            preds_2 = np.where(preds==4, 0, 1)
            targets_2 = np.where(targets==4, 0, 1)
        ...
        else: # task==4 においても, 正常異常の2値分類を行わないため
            return test_acc, f1_score_, report, None, None, None
