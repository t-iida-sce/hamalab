...
class CNN(torch.nn.Module):
    def __init__(self, config,device):
        ...
        if config.cls==1:
            self.fc = nn.Linear(self.feature_dim*config.num_view, config.projection_head)
        elif config.cls==0:
            self.fc = MLPHead(in_channels=self.feature_dim*config.num_view, projection_size=config.projection_head, mid_channels=int(self.feature_dim*config.num_view/3))
        elif config.cls==2:
    ...
    def forward(self, x):
        each_feature = torch.zeros(x.size(0),self.num_view,self.feature_dim).to(self.device)
...
class Net(torch.nn.Module):
    def __init__(self, config,device):
        ...
        if config.name == 'CNN3':
            ...
            self.net = torch.nn.ModuleList(Net18(config) for i in range(config.num_view))
            self.feature_dim = 512
