...
class MFB_one(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        views = views.reshape(views.size(1),-1,views.size(3),views.size(4))
...
class DenseNet(nn.Module):
    def __init__(self, num_view, growthRate, reduction, bottleneck=True):
        ...
        nChannels = 48
        self.conv1 = nn.Conv2d(32*num_view, nChannels, kernel_size=7, padding=1,bias=False)
        ...
        self.dense2 = self._make_dense(nChannels, growthRate, 12, bottleneck)
        nChannels += 12*growthRate
        nOutChannels = 24
        self.trans2 = Transition(nChannels, nOutChannels)

        nChannels = nOutChannels
        self.dense3 = self._make_dense(nChannels, growthRate, 48, bottleneck)
        nChannels += 48*growthRate
        nOutChannels = 24
        self.trans3 = Transition(nChannels, nOutChannels)
        
        nChannels = nOutChannels
        self.dense4 = self._make_dense(nChannels, growthRate, 32, bottleneck)
